...
for i=1:length(metricTypeSet)
    ...
    switch metricType
        case 'error'
            ...
            rankIdx = 21;
            xLabelName = 'Abrupt motion threshold';
            yLabelName = 'AMCR';
    end  
    ...
    for j=1:1
        ...
        dataName = [perfMatPath 'aveSuccessRatePlot_' num2str(numTrk) 'alg_'  plotType '.mat'];%路径 画图文件名
        
        % If the performance Mat file, dataName, does not exist, it will call
        % genPerfMat to generate the file.
        if ~exist(dataName)
            genPerfMatAMCR(seqs, trackers, evalType, nameTrkAll, perfMatPath);
        end        
...
%         draw and save the performance plot for each attribute
        ...
        for attIdx=1:attNum
            ...
            switch metricType
                case 'error'
                    titleName = ['AMCR plots of ' evalType ' - ' attName{attIdx} ' (' num2str(length(idxSeqSet)) ')'];
            end
