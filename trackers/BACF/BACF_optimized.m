...
%     S = getSaliencymap(pixels);
    ...
    while (i <= params.admm_iterations) 
        ...
        S_hx = sum(conj(model_xf) .* h_f, 3);  %S_hx = sum(conj(model_xf) .* f_s, 3)
        ...
        %g_f = (((1/(T*mu)) * bsxfun(@times, yf, model_xf)) - ((1/mu) * l_f) + f_s) - ...
           % bsxfun(@rdivide,(((1/(T*mu)) * bsxfun(@times, model_xf, (S_xx .* yf))) - ((1/mu) * bsxfun(@times, model_xf, S_lx)) + (bsxfun(@times, model_xf, S_hx))), B);
        %   solve for H
        %  f_s = bsxfun(@times,h,S);
        %  f_s = (T/((mu*T)+ params.admm_lambda))* ifft2((mu*g_f) + l_f);
        ...
        %   update L
        l_f = l_f + (mu * (g_f - h_f));%l_f = l_f + (mu * (g_f - f_s))
