...
function[S] = getSaliencymap(pixels)
% clc;
% clear all;
% close all;
addpath('./others/');
...
for ii = 1:length(imnames)
    ...
    [input_im,w]=removeframe(imName);% run a pre-processing to remove the image frame  预处理             
    ...
    % Extract LAB features of all superpixels 提取所有超像素的LAB特征
    [sp_feature_bg] = extractSupfeat(input_imlab,regions_bg,supNum_bg);                                                                                                    
    boundsp_bg = extract_bg_sp(sulabel_bg,m,n); % label the boundary superpixels标记边界超像素
    boundsp_feat_bg = sp_feature_bg(boundsp_bg,:); % Extract the LAB features of the boundary superpixels提取边界超像素的LAB特征
    boundsp_num_bg = length(boundsp_bg);
                       
   %  K-means Algorithm 
    paramPropagate.nclus = 3;    %the number of boundary clusters边界簇的数量
    ...
    % get the labels of boundary superpixels in each cluster获取每个聚类中边界超像素的标签
    bound_clus=cell(1,paramPropagate.nclus);   %cell创建单元格数组
